...
# ---- Abstention instead of a fake "pump" tie-break (2026-07-28) ----


def test_uniform_distribution_abstains_instead_of_defaulting_to_pump():
    """max() returns the FIRST maximal key and every distribution is built
    pump-first, so a source with no directional opinion silently 'predicted
    pump'. A tie must be an abstention."""
    from meme_intelligence.learning.service import _argmax_label

    assert _argmax_label({"pump": 0.25, "flat": 0.25, "dump": 0.25, "rug": 0.25}) is None
    assert _argmax_label({"pump": 0.3, "flat": 0.3, "dump": 0.3, "rug": 0.1}) is None
    assert _argmax_label({"pump": 0.7, "flat": 0.1, "dump": 0.1, "rug": 0.1}) == "pump"
    assert _argmax_label({"pump": 0.1, "flat": 0.1, "dump": 0.1, "rug": 0.7}) == "rug"
    assert _argmax_label({}) is None
    assert _argmax_label(None) is None


def test_rug_engine_never_records_a_directional_call():
    """rug_score_to_distribution spreads non-rug mass UNIFORMLY over
    pump/flat/dump by design — the engine only knows rug vs not-rug. It must
    therefore abstain directionally, never be graded as having called PUMP
    (which depressed its adaptive-ensemble weight on every non-pump coin)."""
    from meme_intelligence.learning.ensemble import rug_score_to_distribution
    from meme_intelligence.learning.service import _argmax_label

    assert _argmax_label(rug_score_to_distribution(0.0)) is None      # clean coin
    assert _argmax_label(rug_score_to_distribution(20.0)) is None     # weak signal
    assert _argmax_label(rug_score_to_distribution(90.0)) == "rug"    # real call


def test_low_information_verdict_is_not_recorded_as_a_prediction():
    """A cold-start coin whose only source is the rug engine has no
    directional opinion: it must not enter the graded population."""
    service = _service()
    service.evaluate_coin("Shrug1", "solana", _pump_series())   # no models trained yet
    coin_id = service.store.coin_id(
        TokenIdentity(chain="solana", address="Shrug1"))
    prediction = service.store.get_prediction(coin_id)
    assert prediction is not None                       # still recorded for audit
    assert prediction["predicted_label"] is None        # but not a graded call
    assert prediction["source_labels"]["rug_engine"] is None
    service.store.close()


def test_min_ensemble_confidence_floor_gates_grading_when_set():
    env = dict(_ENV, MEMEINTEL_LEARNING_MIN_ENSEMBLE_CONFIDENCE="0.99")
    settings = Settings.from_env(env=env)
    store = LearningStore(":memory:", now_func=lambda: NOW)
    service = LearningService(settings, store=store, now_func=lambda: NOW)
    _seed(service)
    service.retrain_if_due()
    service.evaluate_coin("Floor1", "solana", _rug_series())
    prediction = service.store.get_prediction(
        service.store.coin_id(TokenIdentity(chain="solana", address="Floor1")))
    # A confident rug call still exists in the distribution, but a 0.99 floor
    # means the layer declines to be graded on it.
    assert prediction["predicted_label"] is None
    store.close()


def test_abstentions_are_skipped_not_counted_wrong_in_metrics():
    """An abstained verdict must leave the directional denominator alone."""
    service = _service()
    _seed(service)
    service.retrain_if_due()
    service.evaluate_coin("Graded1", "solana", _rug_series())
    service.resolve_outcome("Graded1", "solana", 24.0, -90.0, is_rug=True)
    metrics = service.get_learning_metrics(persist=False)
    assert metrics["directional"]["samples"] >= 0      # no crash on None labels
    service.store.close()
