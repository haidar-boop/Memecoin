...
default 0 = off). Above the ceiling the move already happened, so the
...
### Alerts
- `AlertThresholds` (`ALERTS_`): security 80, community 70, liquidity 70, onchain 75, overall 85, momentum 70; strong_candidate_overall 88 (must be >= overall); strong_candidate_min_liquidity_usd 25000 (depth veto); strong_candidate_min_ai_confidence 40 (AI veto); copycat_veto_enabled true, copycat_liquidity_ratio 10, copycat_min_liquidity_usd 100000; opportunity_min/max_liquidity_usd and min/max_market_cap_usd all **0.0 = OFF** (buy-side floor/ceiling suppressors; unknown data trips the floor but never the ceiling); checklist_sell_tax_max_percent 15, checklist_new_launch_minutes 60 (checklist annotates, doesn't gate — only the rug engine's combined veto suppresses).
